...
void Ipv4::reassembleAndDeliver(Packet *packet)
{
    ...
    if (ipv4Header->getFragmentOffset() != 0 || ipv4Header->getMoreFragments()) {
        //EV_DETAIL << "Datagram fragment: offset=" << ipv4Header->getFragmentOffset()                  << ", MORE=" << (ipv4Header->getMoreFragments() ? "true" : "false") << ".\n";
...
void Ipv4::arpResolutionCompleted(IArp::Notification *entry)
{
    ...
    if (it != pendingPackets.end()) {
        ...
        //EV << "ARP resolution completed for " << entry->l3Address << ". Sending " << packetQueue.getLength()           << " waiting packets from the queue\n";
