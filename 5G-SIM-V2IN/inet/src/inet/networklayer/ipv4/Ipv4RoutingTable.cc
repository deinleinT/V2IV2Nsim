...
void Ipv4RoutingTable::printRoutingTable() const
{
    ...
    for (int i = 0; i < getNumRoutes(); i++) {
        ...
        //EV << stringf("%-16s %-16s %-16s %-4s %-16s %6d\n",                route->getDestination().isUnspecified() ? "*" : route->getDestination().str().c_str(),                route->getNetmask().isUnspecified() ? "*" : route->getNetmask().str().c_str(),                route->getGateway().isUnspecified() ? "*" : route->getGateway().str().c_str(),                !interfacePtr ? "*" : interfacePtr->getInterfaceName(),                !interfacePtr ? "(*)" : (std::string("(") + interfacePtr->getProtocolData<Ipv4InterfaceData>()->getIPAddress().str() + ")").c_str(),                route->getMetric());
    }
    //EV << "\n";
}
