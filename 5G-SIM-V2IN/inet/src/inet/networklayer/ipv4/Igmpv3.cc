...
void Igmpv3::configureInterface(InterfaceEntry *ie)
{
    if (enabled && rt->isMulticastForwardingEnabled()) {
        ...
        //EV_INFO << "Sending General Query on interface '" << ie->getInterfaceName() << "', and scheduling next Query to '"                << (simTime() + startupQueryInterval) << "'.\n";
...
void Igmpv3::processRouterGeneralQueryTimer(cMessage *msg)
{
    ...
    if (state == IGMPV3_RS_QUERIER || state == IGMPV3_RS_NON_QUERIER) {
        //EV_INFO << "Sending General Query on interface '" << ie->getInterfaceName() << "', and scheduling next Query to '"                << (simTime() + queryInterval) << "'.\n";
...
void Igmpv3::processRouterSourceTimer(cMessage *msg)
{
    ...
    //EV_INFO << "Source timer for group '" << groupData->groupAddr << "' and source '" << sourceRecord->sourceAddr            << "' on interface '" << ie->getInterfaceName() << "' has expired.\n";
...
void Igmpv3::processQuery(Packet *packet)
{
    ...
    if (interfaceData->generalQueryTimer->isScheduled() && interfaceData->generalQueryTimer->getArrivalTime() < simTime() + delay) {
        ...
        if (!groupData->timer->isScheduled()) {
            ...
            //EV_DETAIL << "Received Group" << (queriedSources.empty() ? "" : "-and-Source") << "-Specific Query, "                      << "scheduling report with delay=" << delay << ".\n";
            ...
            //EV_DETAIL << "Received Group-Specific Query, scheduling report with delay="                      << min(delay, SIMTIME_DBL(groupData->timer->getArrivalTime() - simTime())) << ".\n";
            ...
            //EV_DETAIL << "Received Group-and-Source-Specific Query, combining sources with the sources of pending report, "                      << "and scheduling a new report with delay="                      << min(delay, SIMTIME_DBL(groupData->timer->getArrivalTime() - simTime())) << ".\n";
            ...
            // RFC 3376 6.4.2: Reception of Filter-Mode-Change and Source-List-Change Records
                ...
                if (groupData->filter == IGMPV3_FM_INCLUDE) {
                    ...
                    if (!aIntersectB.empty()) {
                        //EV_INFO << "Sending Group-and-Source-Specific Query for group '" << groupData->groupAddr                                << "' on interface '" << ie->getInterfaceName() << "'.\n";
                ...
                if (groupData->filter == IGMPV3_FM_INCLUDE) {
                    ...
                    // Send Q(G)
                    //EV_INFO << "Sending Group-Specific Query for group '" << groupData->groupAddr                            << "' on interface '" << ie->getInterfaceName() << "'.\n";
