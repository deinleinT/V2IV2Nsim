...
{
    //EV_INFO << "HOOK " << getFullPath() << ": PREROUTING packet=" << datagram->getName()            // TODO: find out interface name            << " inIE=" << std::to_string(datagram->getTag<InterfaceInd>()->getInterfaceId())            << endl;
    return INetfilter::IHook::ACCEPT;
}

INetfilter::IHook::Result NetfilterInfoHook::datagramForwardHook(Packet *datagram)
{
    //EV_INFO << "HOOK " << getFullPath() << ": FORWARD: packet=" << datagram->getName()            << " inIE=" << std::to_string(datagram->getTag<InterfaceInd>()->getInterfaceId())            << " outIE=" << std::to_string(datagram->getTag<InterfaceReq>()->getInterfaceId())            << " nextHop=" << datagram->getTag<NextHopAddressReq>()->getNextHopAddress()            << endl;
    return INetfilter::IHook::ACCEPT;
}
...
INetfilter::IHook::Result NetfilterInfoHook::datagramPostRoutingHook(Packet *datagram)
{
    ...
    //EV_INFO << "HOOK " << getFullPath() << ": POSTROUTING packet=" << datagram->getName()            << " inIE=" << (interfaceInd ? std::to_string(interfaceInd->getInterfaceId()) : "undefined")            << " outIE=" << std::to_string(datagram->getTag<InterfaceReq>()->getInterfaceId())            << " nextHop=" << datagram->getTag<NextHopAddressReq>()->getNextHopAddress()            << endl;
    return INetfilter::IHook::ACCEPT;
}

INetfilter::IHook::Result NetfilterInfoHook::datagramLocalInHook(Packet *datagram)
{
    //EV_INFO << "HOOK " << getFullPath() << ": LOCAL IN: packet=" << datagram->getName()            << " inIE=" << datagram->getTag<InterfaceInd>()->getInterfaceId()            << endl;
    return INetfilter::IHook::ACCEPT;
}

INetfilter::IHook::Result NetfilterInfoHook::datagramLocalOutHook(Packet *datagram)
{
    auto interfaceReq = datagram->findTag<InterfaceReq>();
    //EV_INFO << "HOOK " << getFullPath() << ": LOCAL OUT: packet=" << datagram->getName()            << " outIE=" << (interfaceReq ? std::to_string(interfaceReq->getInterfaceId()) : "undefined")            << endl;
    return INetfilter::IHook::ACCEPT;
}
