...
void ProbabilisticBroadcast::handleLowerPacket(Packet *packet)
{
    ...
    if (    /*(m->getAppTtl() <= 0) || */ (messageKnown(macHeader->getId()))) {
        ...
        //EV << "PBr: " << simTime() << " n" << myNetwAddr << " handleLowerMsg(): Dead or Known message ID=" << macHeader->getId() << " from node "           << macSrcAddr << " TTL = " << macHeader->getAppTtl() << endl;
        delete packet;
    }
    else {
        ...
        //EV << "PBr: " << simTime() << " n" << myNetwAddr << " handleLowerMsg(): Unknown message ID=" << macHeader->getId() << " from node "           << macSrcAddr << endl;
...
void ProbabilisticBroadcast::insertNewMessage(Packet *packet, bool iAmInitialSender)
{
    ...
    if (ttl > 0) {
        ...
        if (bcastDelay > ttl)
            bcastDelay = ttl;
        //EV << "PBr: " << simTime() << " n" << myNetwAddr << " insertNewMessage(): insert packet " << macHeader->getId() << " with delay "           << bcastDelay << endl;
