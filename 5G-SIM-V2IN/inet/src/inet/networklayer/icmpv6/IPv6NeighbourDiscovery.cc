...
void IPv6NeighbourDiscovery::processIPv6Datagram(IPv6Datagram *msg)
{
    ...
    if (nce == nullptr) {
        ...
        if (ie->isPointToPoint()) {
            ...
            //EV_DETAIL << "Initiating Address Resolution for:" << nextHopAddr                      << " on Interface:" << nextHopIfID << endl;
    ...
    if (ie->ipv6Data()->getAdvSendAdvertisements()) {
        ...
        for (int i = 0; i < (int)ra->getPrefixInformationArraySize(); i++) {
            ...
            if (ra->getHomeAgentFlag() && (prefixInfo.getRouterAddressFlag() == true)) {    //If R-Flag is set and RA is from HA
                ...
                //EV_DETAIL << "The HoA of MN is: " << HoA << ", MN's HA Address is: " << HA                          << " and the home prefix is " << prefixInfo.getPrefix() << endl;
        ...
        IPv6ControlInfo *raCtrlInfo)
{
    ...
    if (neighbour == nullptr) {
        ...
        if (ra->getRouterLifetime() != 0) {
            //EV_INFO << "RA's router lifetime is non-zero, creating an entry in the "                    << "Host's default router list with lifetime=" << ra->getRouterLifetime() << "\n";
    ...
    if (ra->getCurHopLimit() != 0) {
        //EV_INFO << "RA's Cur Hop Limit is non-zero. Setting host's Cur Hop Limit to "                << "received value.\n";
        ...
        InterfaceEntry *ie)
{
    ...
    for (int i = 0; i < (int)ra->getPrefixInformationArraySize(); i++) {
        ...
        //EV_INFO << "Fetching Prefix Information:" << i + 1 << " of "                << ra->getPrefixInformationArraySize() << endl;
        ...
        && !(naMacAddr.isUnspecified()))
    {
        ...
        if (nce->reachabilityState == IPv6NeighbourCache::REACHABLE) {
            ...
            //EV_INFO << "NCE is not in REACHABLE state. Ignore NA.\n";
             ...
             || naMacAddr.isUnspecified())
    {
        //EV_INFO << "NA override flag is TRUE. or Advertised MAC is same as NCE's. or"                << " NA MAC is not specified.\n";
...
void IPv6NeighbourDiscovery::routersUnreachabilityDetection(const InterfaceEntry *ie)
{
    ...
    for (auto it = neighbourCache.begin(); it != neighbourCache.end(); ) {
        if ((*it).first.interfaceID == ie->getInterfaceId() && it->second.isDefaultRouter()) {
            ...
            //EV_INFO << "Setting router (address=" << rtrLnkAddress << ", ifID="                    << (*it).first.interfaceID << ") to unreachable" << endl;
            ++it;
