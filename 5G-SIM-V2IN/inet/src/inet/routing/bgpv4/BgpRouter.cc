...
void BgpRouter::printSessionSummary()
{
    ...
    for(auto &entry : _BGPSessions) {
        ...
        if(type == IGP) {
            //EV_DEBUG << "  IGP session to internal peer '" << session->getPeerAddr().str(false) <<                    "' starts at " << session->getStartEventTime() << "s \n";
        }
        else if(type == EGP) {
            //EV_DEBUG << "  EGP session to external peer '" << session->getPeerAddr().str(false) <<                    "' starts at " << session->getStartEventTime() << "s \n";
        }
        else {
            //EV_DEBUG << "  Unknown session to peer '" << session->getPeerAddr().str(false) <<                    "' starts at " << session->getStartEventTime() << "s \n";
        }
    }
}
...
void BgpRouter::listenConnectionFromPeer(SessionId sessionID)
{
    ...
    if (_BGPSessions[sessionID]->getSocketListen()->getState() != TcpSocket::LISTENING) {
        ...
        //EV_DEBUG << "Start listening to incoming TCP connections on " <<                _BGPSessions[sessionID]->getSocketListen()->getLocalAddress() <<                ":" << (int)TCP_PORT <<                " for " << BgpSession::getTypeString(_BGPSessions[sessionID]->getType()) << " session" <<                " to peer " << _BGPSessions[sessionID]->getPeerAddr() << std::endl;
    }
}

void BgpRouter::openTCPConnectionToPeer(SessionId sessionID)
{
    //EV_DEBUG << "Opening a TCP connection to " <<            _BGPSessions[sessionID]->getPeerAddr() <<            ":" << (int)TCP_PORT << std::endl;
...
void BgpRouter::processMessage(const BgpOpenMessage& msg)
{
    ...
    //EV_INFO << "Processing BGP OPEN message from " <<            session->getPeerAddr().str(false) <<            " with contents: \n";
...
{
    BgpSession *session = _BGPSessions[_currSessionId];
    //EV_INFO << "Processing BGP Keep Alive message from " <<            session->getPeerAddr().str(false) <<            " with contents: \n";
...
{
    BgpSession *session = _BGPSessions[_currSessionId];
    //EV_INFO << "Processing BGP Update message from " <<            session->getPeerAddr().str(false) <<            " with contents: \n";
...
void BgpRouter::updateSendProcess(const unsigned char type, SessionId sessionIndex, BgpRoutingTableEntry *entry)
{
    ...
    for (auto & elem : _BGPSessions)
    {
        ...
        if(entry->isIBgpLearned() && sType == IGP && elem.second->getType() == IGP) {
            //EV_INFO << "BGP Split Horizon: prevent advertisement of network " <<                    entry->getDestination() << "\\" << entry->getNetmask();
            continue;
        }
