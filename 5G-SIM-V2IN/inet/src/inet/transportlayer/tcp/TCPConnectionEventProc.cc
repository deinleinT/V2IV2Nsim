...
void TCPConnection::process_SEND(TCPEventCode& event, TCPCommand *tcpCommand, cMessage *msg)
{
    ...
    switch (fsm.getState()) {
        ...
        case TCP_S_CLOSE_WAIT:
            ...
            //EV_DETAIL << sendQueue->getBytesAvailable(state->snd_una) << " bytes in queue, plus "                      << (state->snd_max - state->snd_una) << " bytes unacknowledged\n";
    ...
    switch (fsm.getState()) {
        ...
        case TCP_S_CLOSE_WAIT:
            ...
            if (state->snd_max == sendQueue->getBufferEndSeq()) {
                ...
                //EV_DETAIL << "SEND of " << (sendQueue->getBufferEndSeq() - state->snd_max)                          << " bytes pending, deferring sending of FIN\n";
                event = TCP_E_IGNORE;
            }
            state->send_fin = true;
