...
veins::BBoxLookup rebuildBBoxLookup(
        const std::vector<std::unique_ptr<veins::Obstacle>> &obstacleOwner,
        int gridCellSize = 250) {
    std::vector<veins::Obstacle*> obstaclePointers;
    obstaclePointers.reserve(obstacleOwner.size());
    std::transform(obstacleOwner.begin(), obstacleOwner.end(),
            std::back_inserter(obstaclePointers),
            [](const std::unique_ptr<veins::Obstacle> &obstacle) {
                return obstacle.get();
            });
    auto playgroundSize =
            veins::FindModule<veins::BaseWorldUtility*>::findGlobalModule()->getPgs();
    auto bboxFunction = [](veins::Obstacle *o) {
        return veins::BBoxLookup::Box { { o->getBboxP1().x, o->getBboxP1().y },
                { o->getBboxP2().x, o->getBboxP2().y } };
    };
    return veins::BBoxLookup(obstaclePointers, bboxFunction, playgroundSize->x,
            playgroundSize->y, gridCellSize);
}
...
void ObstacleControl::initialize(int stage) {
    if (stage == 1) {
        ...
        if (gridCellSize < 1) {
            throw cRuntimeError(
                    "gridCellSize was %d, but must be a positive integer number",
                    gridCellSize);
        }
...
void ObstacleControl::handleMessage(cMessage *msg) {
    ...
    throw cRuntimeError(
            "ObstacleControl doesn't handle messages from other modules");
}
...
void ObstacleControl::addFromXml(cXMLElement *xml) {
    ...
    if (rootTag != "obstacles") {
        throw cRuntimeError(
                "Obstacle definition root tag was \"%s\", but expected \"obstacles\"",
                rootTag.c_str());
    }
    ...
    for (cXMLElementList::const_iterator i = list.begin(); i != list.end();
        ...
        if (tag == "type") {
            ...
            Obstacle obs(id, type, getAttenuationPerCut(type),
                    getAttenuationPerMeter(type));
            std::vector<Coord> sh;
            cStringTokenizer st(shape.c_str());
            while (st.hasMoreTokens()) {
                std::string xy = st.nextToken();
                std::vector<double> xya =
                        cStringTokenizer(xy.c_str(), ",").asDoubleVector();
                ASSERT(xya.size() == 2);
            ...
            throw cRuntimeError(
                    "Found unknown tag in obstacle definition: \"%s\"",
                    tag.c_str());
        }
    }
}

void ObstacleControl::addFromTypeAndShape(std::string id, std::string typeId,
        std::vector<Coord> shape) {
    if (!isTypeSupported(typeId)) {
        throw cRuntimeError("Unsupported obstacle type: \"%s\"",
                typeId.c_str());
    }
    Obstacle obs(id, typeId, getAttenuationPerCut(typeId),
            getAttenuationPerMeter(typeId));
    obs.setShape(shape);
    add(obs);
}
...
void ObstacleControl::add(Obstacle obstacle) {
    ...
    if (annotations)
        o->visualRepresentation = annotations->drawPolygon(o->getShape(), "red",
                annotationGroup);
...
void ObstacleControl::erase(const Obstacle *obstacle) {
    ...
    for (auto itOwner = obstacleOwner.begin(); itOwner != obstacleOwner.end();
            ++itOwner) {
...
std::vector<std::pair<veins::Obstacle*, std::vector<double>>> ObstacleControl::getIntersections(
    ...
    for (Obstacle *o : candidateObstacles) {
        ...
        if (!foundIntersections.empty() || o->containsPoint(senderPos)
                || o->containsPoint(receiverPos)) {
...
double ObstacleControl::calculateAttenuation(const Coord &senderPos,
    ...
    if ((perCut.size() == 0) || (perMeter.size() == 0)) {
        throw cRuntimeError(
                "Unable to use SimpleObstacleShadowing: No obstacle types have been configured");
    }
