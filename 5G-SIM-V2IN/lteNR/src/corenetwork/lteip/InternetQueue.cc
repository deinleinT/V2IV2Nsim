...
{
    //std::cout << "InternetQueue::handleMessage start at " << simTime().dbl() << std::endl;
    ...
    else
    {
        if (endTransmissionEvent_->isScheduled())
        {
            ...
            //EV << "Received " << msg << " for transmission but transmitter busy, queuing." << endl;
    ...
    //std::cout << "InternetQueue::handleMessage end at " << simTime().dbl() << std::endl;
}

void InternetQueue::startTransmitting(cPacket *pkt)
{

    //std::cout << "InternetQueue::startTransmitting start at " << simTime().dbl() << std::endl;

    // Send
    //EV << "Starting transmission of " << pkt << endl;
    send(pkt, queueOutGate_);
    //sendDelayed(pkt, intuniform(1,5)/1000, queueOutGate_);
    ...
    //std::cout << "InternetQueue::startTransmitting end at " << simTime().dbl() << std::endl;
}
