...
void NRsdap::setTrafficInformation(cPacket *pkt, FlowControlInfo *lteInfo) {
	...
	if (strcmp(pkt->getName(), "V2X") == 0 || (applName.find("V2X") != string::npos) || (applName.find("v2x") != string::npos)) {
	...
	} else if (strcmp(pkt->getName(), "VoIP") == 0 || (applName.find("VoIP") != string::npos) || (applName.find("voip") != string::npos) || (applName.find("Voip") != string::npos)) {
		lteInfo->setQfi(qosHandler->getQfi(VOIP));
		lteInfo->setApplication(VOIP);
		lteInfo->setRadioBearerId(qosHandler->getRadioBearerId(lteInfo->getQfi()));
	} else if (strcmp(pkt->getName(), "Video") == 0 || (applName.find("Video") != string::npos) || (applName.find("video") != string::npos)) {
...
NRSdapEntity* NRsdap::getEntity(MacNodeId sender, MacNodeId dest, ApplicationType appType) {
