...
ScheduleListSizes& NRQoSModelScheduler::schedule(MacCid cid, unsigned int availableBytes, Direction grantDir) {
		...
		if (var.second.first == cid) {
			...
			if ((availableBytes > 0) && (queueSizeToServe > 0)) {
				if ((queueSizeToServe <= availableBytes)) {
					...
					scheduleListSizes_[cid].second = bufferInfo->sentData_;
					...
					scheduleListSizes_[cid].first = bufferInfo->sentSdus_;
