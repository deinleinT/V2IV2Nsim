	...
	if (stage == inet::INITSTAGE_PHYSICAL_ENVIRONMENT) {
		...
		if (dynamicCellAssociation_) {
			...
			for (; it != enbList->end(); ++it) {
				...
				double rssi = 0;
				std::vector<double> rssiV = check_and_cast<NRRealisticChannelModel*>(primaryChannelModel_)->getSINR(frame, cInfo, false);
...
void NRPhyUE::handoverHandler(LteAirFrame * frame, UserControlInfo * lteInfo) {
	...
	if ((getNodeTypeById(lteInfo->getSourceId()) == ENODEB || getNodeTypeById(lteInfo->getSourceId()) == GNODEB)
		...
		std::vector<double>::iterator it;
		rssi = 0;
		std::vector<double> rssiV = check_and_cast<NRRealisticChannelModel*>(primaryChannelModel_)->getSINR(frame, lteInfo, false);
		for (it = rssiV.begin(); it != rssiV.end(); ++it)
			rssi += *it;
		rssi /= rssiV.size();
	}
...
void NRPhyUE::checkConnection() {
	...
	if (getSystemModule()->hasPar("useSINRThreshold")) {
		if (getSystemModule()->par("useSINRThreshold").boolValue()) {
			...
			if (maxRssiUL <= std::min(minRssi_, double(getSimulation()->getSystemModule()->par("SINRThreshold").intValue()))) {
			...
			scheduleAt(simTime() + getSimulation()->getSystemModule()->par("checkConnectionInterval").doubleValue() + uniform(0, 0.005),
...
void NRPhyUE::triggerHandover() {
	...
	if (masterId_ != 0 && candidateMasterId_ != 0) {
	    IP2NR *enbIp2nic = check_and_cast<IP2NR*>(getSimulation()->getModule(binder_->getOmnetId(masterId_))->getSubmodule("cellularNic")->getSubmodule("ip2nic"));
...
void NRPhyUE::doHandover() {
	...
	if (candidateMasterId_ != 0) {
		...
		LteMacEnb *newMacEnb = check_and_cast<LteMacEnb*>(
				getSimulation()->getModule(binder_->getOmnetId(candidateMasterId_))->getSubmodule("cellularNic")->getSubmodule("mac"));
	...
	if (oldMaster != 0 && candidateMasterId_ != 0) {
		IP2NR *enbIp2nic = check_and_cast<IP2NR*>(
				getSimulation()->getModule(binder_->getOmnetId(masterId_))->getSubmodule("cellularNic")->getSubmodule("ip2nic"));
...
void NRPhyUE::deleteOldBuffers(MacNodeId masterId) {
	...
	LtePhyBase *masterPhy = check_and_cast<LtePhyBase*>(getSimulation()->getModule(masterOmnetId)->getSubmodule("cellularNic")->getSubmodule("phy"));
	...
	NRMacGNB *masterMac = check_and_cast<NRMacGNB*>(getSimulation()->getModule(masterOmnetId)->getSubmodule("cellularNic")->getSubmodule("mac"));
	...
	//qosHandler UE
	check_and_cast<NRMacUE*>(mac_)->deleteNodeFromQosHandler(masterId_);

	/////////////////////////////////////////////////////////////////////////////////

	/* Delete Rlc UM Buffers */
	LteRlcUm *masterRlcUmRealistic = check_and_cast<LteRlcUm*>(
			getSimulation()->getModule(masterOmnetId)->getSubmodule("cellularNic")->getSubmodule("rlc")->getSubmodule("um"));
	...
	NRsdapGNB *sdapGNB = check_and_cast<NRsdapGNB*>(getSimulation()->getModule(masterOmnetId)->getSubmodule("cellularNic")->getSubmodule("sdap"));
