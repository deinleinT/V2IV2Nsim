...
public:
	...
	//converts omnet coordinates to sumo
	inet::Coord omnet2sumo(double x, double y) const {
		return inet::Coord(x + topleft.x - margin, dimensions.y - (y - topleft.y) + margin);
	}

	//converts sumo coordinates to omnet
	inet::Coord sumo2omnet(double x, double y) const {
		return inet::Coord(x - topleft.x + margin, dimensions.y - (y - topleft.y) + margin);
	}
...
protected:
	...
	//SUMO VALUES
	float margin;
	inet::Coord dimensions;
	inet::Coord topleft;
	inet::Coord bottomright;
	//
