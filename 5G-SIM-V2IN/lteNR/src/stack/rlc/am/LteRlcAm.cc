...
LteRlcAm::getTxBuffer(MacNodeId nodeId, LogicalCid lcid)
{
    ...
    if (it == txBuffers_.end())
    {
        ...
        //EV << NOW << " LteRlcAm : Added new AmTxBuffer: " << txbuf->getId() << " for node: " << nodeId << " for Lcid: " << lcid << "\n";
    ...
    {
        // Found
        //EV << NOW << " LteRlcAm : Using old AmTxBuffer: " << it->second->getId() << " for node: " << nodeId << " for Lcid: " << lcid << "\n";
...
LteRlcAm::getRxBuffer(MacNodeId nodeId, LogicalCid lcid)
{
    ...
    if (it == rxBuffers_.end())
    {
        ...
        //EV << NOW << " LteRlcAm : Added new AmRxBuffer: " << rxbuf->getId() << " for node: " << nodeId << " for Lcid: " << lcid << "\n";
    ...
    {
        // Found
        //EV << NOW << " LteRlcAm : Using old AmRxBuffer: " << it->second->getId() << " for node: " << nodeId << " for Lcid: " << lcid << "\n";
...
void LteRlcAm::sendDefragmented(cPacket *pkt)
{
    ...
    //EV << NOW << " LteRlcAm : Sending packet " << pkt->getName() << " to port AM_Sap_up$o\n";
    send(pkt, up_[OUT]);
}
...
void LteRlcAm::sendFragmented(cPacket *pkt)
{
    ...
    //EV << NOW << " LteRlcAm : Sending packet " << pkt->getName() << " of size " << pkt->getByteLength() << "  to port AM_Sap_down$o\n";
...
void LteRlcAm::handleMessage(cMessage* msg)
{
    ...
    //EV << NOW << " LteRlcAm : Received packet " << pkt->getName() << " from port " << pkt->getArrivalGate()->getName() << endl;
