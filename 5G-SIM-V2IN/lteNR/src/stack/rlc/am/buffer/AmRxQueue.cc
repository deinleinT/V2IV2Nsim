...
void AmRxQueue::enque(LteRlcAmPdu* pdu)
{
    ...
    if (pdu->getAmType() != DATA)
    {
        ...
        {
            //EV << NOW << " AmRxQueue::enque MRW command [" << pdu->getSnoMainPacket() << "] received for sequence number  " << pdu->getLastSn() << endl;
    ...
    else
    {
        ...
        else
        {
            ...
            //EV << NOW << "AmRxQueue::enque DATA PDU received at index [" << index << "] with fragment number [" << tsn << "] out of sequence, sending status report " << endl;
            sendStatusReport();
        }
...
void AmRxQueue::checkCompleteSdu(const int index)
{
    ...
    if (!bComplete)
    {
        //EV << NOW << " AmRxQueue::checkCompleteSdu - SDU cannot be reconstructed, backward search didn't found any predecessors to PDU at " << index << endl;
        return;
    }
        // search ended with current PDU, the whole SDU can be reconstructed.
    if (pdu->isLast())
    {
        //EV << NOW << " AmRxQueue::checkCompleteSdu - complete SDU has been found, backward search was successful, and current was last of its SDU"" passing up " << firstIndex << endl;
        passUp(firstIndex);
        return;
    }
    ...
    for (int i = index + 1; i < (rxWindowDesc_.windowSize_); ++i)
    {
        if (received_.at(i) == false)
        {
            //EV << NOW << " AmRxQueue::checkCompleteSdu forward search failed, no PDU at position " << i << " corresponding to"" SN  " << i+rxWindowDesc_.firstSeqNum_ << endl;
        ...
        {
            complete = true;
            //EV << NOW << " AmRxQueue::checkCompleteSdu: forward search successful, last PDU found at position " << i << endl;
    ...
    if (complete == true)
    {
        //EV << NOW << " AmRxQueue::checkCompleteSdu - complete SDU has been found after forward search , passing up " << firstIndex << endl;
        passUp(firstIndex);
        return;
    }
}
...
void AmRxQueue::sendStatusReport()
{
    ...
    {
        //EV << NOW << " AmRxQueue::sendStatusReport , minimum interval not reached "<< ackReportInterval_.dbl() << endl;
    ...
    //EV << NOW << " AmRxQueue::sendStatusReport : cumulative ACK value " << cumulative << " bitmap lenght " << bitmap.size() << endl;
    ...
    if ((cumulative > 0) || bitmap.size() > 0)
    {
        ...
        // set bitmap
        //EV << NOW << " AmRxQueue::sendStatusReport : sending the cumulative ACK for  " << lastSn << endl;
    ...
    {
        //EV << NOW << " AmRxQueue::sendStatusReport : NOT sending the cumulative ACK for  " << cumulative << endl;
    }
}
...
void AmRxQueue::moveRxWindow(const int seqNum)
{
    ...
    //EV << NOW << " AmRxQueue::moveRxWindow first sequence number updated to " << rxWindowDesc_.firstSeqNum_ << endl;

    firstSdu_ = currentSdu;
    //EV << NOW << " AmRxQueue::moveRxWindow current SDU updated to " << firstSdu_ << endl;
}
