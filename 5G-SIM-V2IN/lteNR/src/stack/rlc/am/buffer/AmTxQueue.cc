...
void AmTxQueue::addPdus()
{
    ...
    while ((txWindowDesc_.seqNum_ - txWindowDesc_.firstSeqNum_) < txWindowDesc_.windowSize_)
    {
        ...
        if (currentSdu_ == NULL)
        {
            ...
            //EV << NOW << " AmTxQueue::addPdus current SDU size " << currentSdu_->getByteLength() << " will be fragmented in " << fragDesc_.totalFragments_ << " PDUs, each  of size " << fragDesc_.fragUnit_ << endl;
...
void AmTxQueue::discard(const int seqNum)
{
    ...
    //EV << NOW << " AmTxQueue::discard sequence number [" << seqNum << "] window index [" << txWindowIndex << "]" << endl;
    ...
    {
        //EV << " AmTxQueue::discard requested to discard an already discarded  PDU :"" sequence number" << seqNum << " , window first sequence is " << txWindowDesc_.firstSeqNum_ << endl;
    }
    else
    {
...
void AmTxQueue::moveTxWindow(const int seqNum)
{
    ...
    //EV << NOW << " AmTxQueue::moveTxWindow sequence number " << seqNum << " corresponding index " << pos << endl;
    ...
    for (int i = 0; i < pos; ++i)
    {
        if (pduRtxQueue_.get(i) != NULL)
        {
            //EV << NOW << " AmTxQueue::moveTxWindow deleting PDU [" << i + txWindowDesc_.firstSeqNum_ << "] corresponding index " << i << endl;
            ...
            {
                pduTimer_.remove(i + txWindowDesc_.firstSeqNum_);
                //EV << NOW << " AmTxQueue::moveTxWindow canceling PDU timer [" << i + txWindowDesc_.firstSeqNum_ << "] corresponding index " << i << endl;
            }
        ...
        i < ((txWindowDesc_.seqNum_ - txWindowDesc_.firstSeqNum_)); ++i)
    {
        if (pduRtxQueue_.get(i) != NULL)
        {
            ...
            //EV << NOW << " AmTxQueue::moveTxWindow  PDU [" << i + txWindowDesc_.firstSeqNum_  << "] corresponding index " << i << " being moved at position " << i - pos << endl;
        }
        else
        {
            throw cRuntimeError("AmTxQueue::moveTxWindow(): encountered empty PDU at location %d, shift position %d", i, pos);
        ...
        //EV << NOW << " AmTxQueue::moveTxWindow  PDU [" << i + txWindowDesc_.firstSeqNum_ << "] corresponding new index " << i - pos  << " marked as received [" << (received_.at(i - pos)) << "] , discarded [" << (discarded_.at(i - pos)) << "]" << endl;
        ...
        //EV << NOW << " AmTxQueue::moveTxWindow  location [" << i << "] corresponding new PDU " << i + pos + txWindowDesc_.firstSeqNum_ << " marked as received [" << (received_.at(i)) << "] , discarded [" << (discarded_.at(i)) << "]" << endl;
    }
        ...
        i < txWindowDesc_.windowSize_; ++i)
    {
        ...
        //EV << NOW << " AmTxQueue::moveTxWindow  empty location [" << i << "] marked as received [" << (received_.at(i)) << "] , discarded [" << (discarded_.at(i)) << "]" << endl;
    ...
    //EV << NOW << " AmTxQueue::moveTxWindow completed. First sequence number " << txWindowDesc_.firstSeqNum_ << " current sequence number " << txWindowDesc_.seqNum_ << " cleaned up locations from " << txWindowDesc_.seqNum_ - txWindowDesc_.firstSeqNum_ << " to " << txWindowDesc_.windowSize_ - 1 << endl;
...
{
    //EV << NOW << " AmTxQueue::sendMrw sending MRW PDU [" << mrwDesc_.mrwSeqNum_ << "]for sequence number " << seqNum << endl;
...
void AmTxQueue::handleControlPacket(cPacket* pkt)
{
    ...
    {
        case MRW_ACK:
            //EV << NOW << " AmTxQueue::handleControlPacket , received MRW ACK [" << pdu->getSnoMainPacket() << "]: window new first SN  " << pdu->getSnoFragment() << endl;
            // move tx window
            ...
            if (bSize > 0)
            {
                //EV << NOW << " AmTxQueue::handleControlPacket , received BITMAP ACK of size " << bSize << endl;
...
void AmTxQueue::recvAck(const int seqNum)
{
    ...
    //EV << NOW << " AmTxBuffer::recvAck ACK received for sequence number " << seqNum << " first sequence n. [" << txWindowDesc_.firstSeqNum_ << "] index [" << index << "] " << endl;

    if (index < 0)
    {
        //EV << NOW << " AmTxBuffer::recvAck ACK already received - ignoring : index " << index << " first sequence number" << txWindowDesc_.firstSeqNum_ << endl;
    ...
    {
        //EV << NOW << " AmTxBuffer::recvAck canceling timer for PDU " << (index + txWindowDesc_.firstSeqNum_) << " index " << index << endl;
        // Stop the timer
...
void AmTxQueue::recvCumulativeAck(const int seqNum)
{
    ...
    else
    {
        ...
        {
            //EV << NOW << " AmTxBuffer::recvCumulativeAck ACK received for sequence number " << (i + txWindowDesc_.firstSeqNum_) << " first sequence n. [" << txWindowDesc_.firstSeqNum_ << "] " "index [" << i << "] " << endl;

            // the ACK could have already been received
            if (!(received_.at(i)))
            {
                // canceling timer for PDU
                //EV << NOW << " AmTxBuffer::recvCumulativeAck canceling timer for PDU " << (i + txWindowDesc_.firstSeqNum_) << " index " << i << endl;
                // Stop the timer
                if (pduTimer_.busy(i + txWindowDesc_.firstSeqNum_))
                pduTimer_.remove(i + txWindowDesc_.firstSeqNum_);
                // Received status variable is set at true after the
