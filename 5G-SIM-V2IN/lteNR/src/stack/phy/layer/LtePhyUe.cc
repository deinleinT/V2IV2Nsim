...
void LtePhyUe::initialize(int stage)
{
    ...
    if (stage == inet::INITSTAGE_LOCAL)
    {
        ...
        exchangeBuffersOnHandover_ = par("exchangeBuffersOnHandover").boolValue();
...
void LtePhyUe::handleSelfMessage(cMessage *msg) {
    ...
    //std::cout << "LtePhyUe::handleSelfMessage start at " << simTime().dbl() << std::endl;
    ...
    //std::cout << "LtePhyUe::handleSelfMessage end at " << simTime().dbl() << std::endl;
}

void LtePhyUe::handoverHandler(LteAirFrame* frame, UserControlInfo* lteInfo) {

    //std::cout << "LtePhyUe::handoverHandler start at " << simTime().dbl() << std::endl;
    ...
    if (!enableHandover_)
    {
        ...
        if ((getNodeTypeById(lteInfo->getSourceId()) == ENODEB
                || getNodeTypeById(lteInfo->getSourceId()) == GNODEB)
        ...
        delete frame;
        delete lteInfo;
        //std::cout << "LtePhyUe::handoverHandler end at " << simTime().dbl() << std::endl;
        return;
    }
...
void LtePhyUe::triggerHandover() {
    ...
    //std::cout << "LtePhyUe::triggerHandover start at " << simTime().dbl() << std::endl;
    ...
    if (masterId_ == candidateMasterId_) {
        //std::cout << "TriggerHandover at " << simTime().dbl() << " oldMaster: " << masterId_ << " newMaster " << candidateMasterId_ << " nodeId " << nodeId_ << std::endl;
        return;
    }

    scheduleAt(simTime() + handoverLatency_, handoverTrigger_);
    //std::cout << "LtePhyUe::triggerHandover end at " << simTime().dbl() << std::endl;
}

void LtePhyUe::doHandover() {

    //std::cout << "LtePhyUe::doHandover start at " << simTime().dbl() << std::endl;

    //
    if(exchangeBuffersOnHandover_)
        exchangeBuffersOnHandover(masterId_, candidateMasterId_);
    else
    ...
    //std::cout << "LtePhyUe::doHandover finished at " << simTime().dbl() << " masterId: " << masterId_ << " candidateMasterID " << candidateMasterId_ << " nodeId " << nodeId_ << std::endl;
}
...
void LtePhyUe::handleAirFrame(cMessage* msg) {
    ...
    //std::cout << "LtePhyUe::handleAirFrame start at " << simTime().dbl() << std::endl;
...
//            m.setStart(das_->getAntennaCoord(*it));
    ...
    //std::cout << "LtePhyUe::handleAirFrame end at " << simTime().dbl() << std::endl;
}
...
//    }
    ...
    //std::cout << "LtePhyUe::handleUpperMessage start at " << simTime().dbl() << std::endl;
    ...
    if (dest != masterId_) {
        ...
        //EV << "ERROR: Ue preparing to send message to " << dest << "instead ""of its master (" << masterId_ << ")" << endl;
        endSimulation();
    }
    ...
    //std::cout << "LtePhyUe::handleUpperMessage end at " << simTime().dbl() << std::endl;
}
...
void LtePhyUe::deleteOldBuffers(MacNodeId masterId) {
    ...
    //std::cout << "LtePhyUe::deleteOldBuffers start at " << simTime().dbl() << std::endl;
    ...
    LteMacEnb *masterMac =
            check_and_cast<LteMacEnb *>(
                    getSimulation()->getModule(masterOmnetId)->getSubmodule(
                            "lteNic")->getSubmodule("mac"));
    ...
    LteMacUe * macReal = check_and_cast<LteMacUe*>(mac_);
    macReal->resetScheduleList();
    //


    // delete UmTxQueue[nodeId_] at old master
    LteRlcUm *masterRlcUm =
            check_and_cast<LteRlcUm *>(
                    getSimulation()->getModule(masterOmnetId)->getSubmodule(
                            "lteNic")->getSubmodule("rlc")->getSubmodule("um"));
    masterRlcUm->deleteQueues(nodeId_);

    // delete queues for master at this ue
    rlcUm_->deleteQueues(nodeId_);

    //TODO pdcp_rrc --> connectionTable reset, masterId == oldENB --> delete entry
    // nodeId_ --> delete whole
    NRPdcpRrcGnb *masterPdcp =
                check_and_cast<NRPdcpRrcGnb *>(
                        getSimulation()->getModule(masterOmnetId)->getSubmodule(
                                "lteNic")->getSubmodule("pdcpRrc"));

    masterPdcp->resetConnectionTable(masterId, nodeId_);

    //std::cout << "LtePhyUe::deleteOldBuffers end at " << simTime().dbl() << std::endl;
}

DasFilter* LtePhyUe::getDasFilter() {
    Enter_Method_Silent("getDasFilter");
    return das_;
}

void LtePhyUe::sendFeedback(LteFeedbackDoubleVector fbDl,
        LteFeedbackDoubleVector fbUl, FeedbackRequest req) {

    Enter_Method_Silent("SendFeedback");

    //std::cout << "LtePhyUe::sendFeedback start at " << simTime().dbl() << std::endl;
...
//    }
    lastFeedback_ = NOW;
    //EV << "LtePhy: " << nodeTypeToA(nodeType_) << " with id " << nodeId_ << " sending feedback to the air channel" << endl;
    sendUnicast(frame);

    //std::cout << "LtePhyUe::sendFeedback end at " << simTime().dbl() << std::endl;
}
