...
bool LteMacUe::bufferizePacket(cPacket* pkt)
{
    ...
    else
    {
		...
		EV << "LteMacBuffers : Using old buffer on node: " << MacCidToNodeId(cid) << " for Lcid: " << MacCidToLcid(cid) << "(cid: " << cid << "), Space left in the Queue: " << queue->getQueueSize() - queue->getByteLength() << "\n";
    }
...
{
    //std::cout << "LteMacUe macPduMake start at " << simTime().dbl() << std::endl;
    ...
    for (pit = macPduList_.begin(); pit != macPduList_.end(); pit++)
    {
        ...
        HarqTxBuffers::iterator hit = harqTxBuffers_.find(cellId_);
...
//        LteHarqProcessTx * currProc = txBuf->getProcess(currentHarq_);
    ...
    //std::cout << "LteMacUe macPduMake end at " << simTime().dbl() << std::endl;
}
...
{
    //std::cout << "LteMacUe handleSelfMessage start at " << simTime().dbl() << std::endl;
...
//        }
    ...
    //EV << "\n htxbuf.size " << harqTxBuffers_.size() << endl;
		...
		for(; jt != jet; ++jt)
		{
		    //EV << "\t\t cicloInner " << cntInner << " - jt->size=" << jt->size() << " - statusCw(0/1)=" << jt->at(0).second << "/" << jt->at(1).second << endl;
		}
    }
    }
    //======================== END DEBUG ==========================
    ...
    //std::cout << "LteMacUe handleSelfMessage end at " << simTime().dbl() << std::endl;
}

void
LteMacUe::macHandleGrant(cPacket* pkt)
{
    //std::cout << "LteMacUe macHandleGrant start at " << simTime().dbl() << std::endl;
...
{
    //std::cout << "LteMacUe macHandleRac start at " << simTime().dbl() << std::endl;
    ...
    //std::cout << "LteMacUe macHandleRac end at " << simTime().dbl() << std::endl;
    delete racPkt;
}
...
LteMacUe::checkRAC()
{
    //std::cout << "LteMacUe checkRAC start at " << simTime().dbl() << std::endl;

    //EV << NOW << " LteMacUe::checkRAC , Ue  " << nodeId_ << ", racTimer : " << racBackoffTimer_ << " maxRacTryOuts : " << maxRacTryouts_ << ", raRespTimer:" << raRespTimer_ << endl;
    ...
    for (it = macBuffers_.begin(); it!=macBuffers_.end();++it)
    {
        ...
        {//TODO --> sort the buffer in order the qfi
            trigger = true;
            break;
        }
    }
    ...
    if ((racRequested_=trigger))
    {
        ...
        uinfo->setCid(macBuffers_.begin()->first);
    ...
    //std::cout << "LteMacUe checkRAC end at " << simTime().dbl() << std::endl;
}

void
LteMacUe::updateUserTxParam(cPacket* pkt)
{
    //std::cout << "LteMacUe updateUserTxParam start at " << simTime().dbl() << std::endl;
    ...
    //std::cout << "LteMacUe updateUserTxParam end at " << simTime().dbl() << std::endl;
}
...
{
    //std::cout << "LteMacUe doHandover at " << simTime().dbl() << std::endl;
    cellId_ = targetEnb;
}

void LteMacUe::deleteQueues(MacNodeId nodeId)
{
    //std::cout << "LteMacUe deleteQueues start at " << simTime().dbl() << std::endl;
    ...
    //std::cout << "LteMacUe deleteQueues end at " << simTime().dbl() << std::endl;
}

void LteMacUe::changeMasterId(MacNodeId oldMasterId, MacNodeId newMasterId){

    //std::cout << "LteMacUe deleteQueues start at " << simTime().dbl() << std::endl;

    //no changes in the buffer, cid remains
    ...
    // remove traffic descriptor and lcg entry
    //lcgMap_.clear();--> should be no changes
    ...
    for (auto & var : nodeQosInfos) {
        ...
        if (var.second.destNodeId == oldMasterId) {
            var.second.destNodeId = newMasterId;
        }
    }
    qosHandler->getQosInfo() = nodeQosInfos;

    //std::cout << "LteMacUe changeMasterId end at " << simTime().dbl() << std::endl;
}
