...
public:
	virtual void insertRtxMap(MacNodeId ueid, unsigned char currentProcess, Codeword codeword) {
		rtxMap[ueid].insert(std::make_pair(currentProcess,codeword));
	}
    ...
    void resetScheduleList(){ scheduleListDl_->clear();};
  ...
  protected:
    std::map<MacNodeId, std::set<std::pair<unsigned char,Codeword>>> rtxMap;
...
//        std::vector<LteSubFrameType> mbsfnPattern_;
  ...
  public:
    ...
    {
        Enter_Method_Silent("getBsrVirtualBuffers");
