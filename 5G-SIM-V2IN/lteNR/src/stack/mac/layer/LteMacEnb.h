...
public:
	virtual void insertRtxMap(MacNodeId ueid, unsigned char currentProcess) {
		rtxMap[ueid].insert(currentProcess);
	}

	virtual void eraseRtxMap(MacNodeId ueid, unsigned char currentProcess) {
		rtxMap[ueid].erase(rtxMap[ueid].find(currentProcess));
	}
...
protected:
	std::map<MacNodeId, std::set<unsigned char>> rtxMap;
