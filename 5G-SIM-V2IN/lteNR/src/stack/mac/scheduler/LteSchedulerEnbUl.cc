...
//
bool LteSchedulerEnbUl::checkEligibility(MacNodeId id, Codeword& cw)
{

    //std::cout << "LteSchedulerEnbUl::checkEligibility start at " << simTime().dbl() << std::endl;
    ...
    //std::cout << "LteSchedulerEnbUl::checkEligibility end at " << simTime().dbl() << std::endl;
...
{
    //std::cout << "LteSchedulerEnbUl::updateHarqDescs start at " << simTime().dbl() << std::endl;
    ...
    //std::cout << "LteSchedulerEnbUl::updateHarqDescs end at " << simTime().dbl() << std::endl;
}
...
bool LteSchedulerEnbUl::racschedule()
{
    ...
    for (;it!=et;++it)
    {
        ...
        if (allocation)
        {
            ...
            scheduleList_[scListId].first=blocks;
        }
    }
...
LteSchedulerEnbUl::rtxschedule()
{
    ...
    //std::cout << "LteSchedulerEnbUl::rtxschedule start at " << simTime().dbl() << std::endl;
    ...
    try
    {
        ...
        //std::cout << "LteSchedulerEnbUl::rtxschedule end at " << simTime().dbl() << std::endl;
...
{
    //std::cout << "LteSchedulerEnbUl::schedulePerAcidRtx start at " << simTime().dbl() << std::endl;
...
//                    ((allocatedCw == MAX_CODEWORDS) ? availableBytes(nodeId,antenna, b, cw) : mac_->getAmc()->blocks2bytes(nodeId, b, cw, allocator_->getBlocks(antenna,b,nodeId) , direction_));    // available space
        ...
        else
        {
            ...
            //std::cout << "LteSchedulerEnbUl::schedulePerAcidRtx end at " << simTime().dbl() << std::endl;
...
LteSchedulerEnbUl::schedulePerAcidRtxD2D(MacNodeId destId,MacNodeId senderId, Codeword cw, unsigned char acid,
    std::vector<BandLimit>* bandLim, Remote antenna, bool limitBl)
{
    //std::cout << "LteSchedulerEnbUl::schedulePerAcidRtxD2D start at " << simTime().dbl() << std::endl;
...
//            }
//            unsigned int servedBlocks = mac_->getAmc()->computeReqRbs(senderId, b, cw, servedBytes, dir, allocator_->availableBlocks(senderId,antenna,b));
//            // update the bytes counter
//            toServe -= servedBytes;
//            // update the structures
//            assignedBlocks.push_back(servedBlocks);
...
{
    //std::cout << "LteSchedulerEnbUl::initHarqStatus start at " << simTime().dbl() << std::endl;

    //EV << NOW << ")LteSchedulerEnbUl::initHarqStatus - initializing harq status for id " << id << "to status " << (unsigned int)acid << endl;
    harqStatus_[id]=acid;

    //std::cout << "LteSchedulerEnbUl::initHarqStatus end at " << simTime().dbl() << std::endl;
}

void LteSchedulerEnbUl::removePendingRac(MacNodeId nodeId)
{
    //std::cout << "LteSchedulerEnbUl::removePendingRac start at " << simTime().dbl() << std::endl;

    racStatus_.erase(nodeId);

    //std::cout << "LteSchedulerEnbUl::removePendingRac end at " << simTime().dbl() << std::endl;
}
