    ...
    for (; it != et; it++)  // for each antenna
        ...
        for (; uit != uet; uit++) // for each UE
            ...
            for (; txit != txet; txit++)  // for each tx mode
                ...
                ((*txit).get()).print(0, (*revIndex)[i], dir, txMode,
                        "LteAmc::printAmcFbhb");
            }
            i++;
        }
    ...
    for (; it != et; it++)  // For all UEs (UL)
        ...
        for (; ait != aet; ait++) {
            ...
            ulFeedbackHistory_[*ait].push_back(
                    std::vector<LteSummaryBuffer>(UL_NUM_TXMODE,LteSummaryBuffer(fbhbCapacityUl_, MAXCW, numBands_,lb_, ub_)));
        }
    }
...
const UserTxParams& LteAmc::setTxParams(MacNodeId id, const Direction dir,
        UserTxParams& info) {
...
unsigned int LteAmc::computeReqRbs(MacNodeId id, Band b, Codeword cw,
        unsigned int bytes, const Direction dir, unsigned int availableBlocks) {
    ...
    for(unsigned int i = 1; i <= availableBlocks; i++){
    	unsigned int tbs = calcTBS(id, i, mcsIndex, layers);
    	if(tbs >= bitsToSend){
    		numRBs = i;
    		break;
    	}
...
//changed for 5G-SIM-V2I/N
unsigned int LteAmc::computeBitsOnNRbs(MacNodeId id, Band b,
        unsigned int blocks, const Direction dir) {
    ...
    for (Codeword cw = 0; cw < codewords; ++cw) {
        ...
        //Changed
        unsigned int mcsIndex = getMcsIndexCqi(info.readCqiVector().at(cw), dir);
        unsigned int tbs = calcTBS(id, blocks, mcsIndex, info.getLayers().at(cw));
...
unsigned int LteAmc::computeBytesOnNRbs(MacNodeId id, Band b,
        unsigned int blocks, const Direction dir) {
...
unsigned int LteAmc::computeBytesOnNRbs(MacNodeId id, Band b, Codeword cw,
        unsigned int blocks, const Direction dir) {
...
unsigned int LteAmc::computeBytesOnNRbs_MB(MacNodeId id, Band b,
        unsigned int blocks, const Direction dir) {
...
unsigned int LteAmc::computeBitsOnNRbs_MB(MacNodeId id, Band b,
        unsigned int blocks, const Direction dir) {
    ...
    unsigned int tbs = calcTBS(id, blocks, mcsIndex, layers[0]);
...
bool LteAmc::setPilotUsableBands(MacNodeId id,
        std::vector<unsigned short> usableBands) {
    ...
    if (dir == UL || dir == DL) {
        ...
        for (; it != et; it++) {
            ...
            for (int i = 0; i < numTxModes; i++) {
                // Print only non empty feedback summary! (all cqi are != NOSIGNALCQI)
                Cqi testCqi = (feedback.at(i).get()).getCqi(Codeword(0),
                        Band(0));
                if (testCqi == NOSIGNALCQI)
                    continue;

                feedback.at(i).get().print(0, nodeId, dir, TxMode(i),
                        "LteAmc::testUe");
            }
        }
    } else // D2D
